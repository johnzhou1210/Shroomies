...
public class StageLogic : MonoBehaviour
{
    ...
    [SerializeField] GameObject _upgradeFrame,
        _uiCanvas,
        _playerDragArea,
        _buyShroomieButton,
        _gameOverEffect,
    ...
    IEnumerator BeginRoguelikeRun()
    {
        ...
        while (StageNumber <= _numStagesPerWorldIncludingBoss)
        {
            ...
            {
                AudioManager.Instance.PlayMistaDJ();
            } //{ AudioManager.Instance.PlayMusic("s Next Spread"); }

            OnCueStageBanner?.Invoke(StageNumber == _bossStage || StageNumber == _bossStage2
                ? "<color=#" + ChangePalette.Holder.color2.ToHexString() + ">" + WorldNumber + "-" + StageNumber +
                  "</color>"
                : WorldNumber + "-" + StageNumber);

            yield return new WaitForSeconds(_stageBeginWaitDelay);

            // choose which collection to use to spawn with
            ClusterCollection chosenCollection = Array.Find(_clusterCollections,
                c => c.WorldNumber == WorldNumber && c.StageNumber == StageNumber);
            ...
            if (StageNumber != _bossStage && StageNumber != _bossStage2)
            {
                ...
                while (currNumClustersElapsed < numClustersToSpawn && !_gameOver)
                {
                    ...
                    GameObject chosenClusterPrefabToSpawn =
                        Instantiate(
                            chosenCollection.Clusters[UnityEngine.Random.Range(0, chosenCollection.Clusters.Length)],
                            GameObject.FindWithTag("EnemyContainer").transform);
                    ...
                    yield return new WaitForSeconds(UnityEngine.Random.Range(currClusterSettings.NextClusterMinDelay,
                        currClusterSettings.NextClusterMaxDelay / (Mathf.Pow(1.1f, 1.2f * Difficulty) - .4f)));
            ...
            else
            {
                ...
                GameObject bossClusterPrefab = Instantiate(chosenCollection.Clusters[0],
                    GameObject.FindWithTag("EnemyContainer").transform);
                AddEnemyListeners(bossClusterPrefab.transform.GetChild(0), Difficulty);
                yield return new WaitForSeconds(3f);
                // cue boss banner
                OnCueBossBanner?.Invoke(bossClusterPrefab.transform.GetChild(0).GetComponent<DisplayData>()
                    .DisplayInfo);
            ...
            if (_gameOver)
            {
                // make sure it's not over because the player died
                yield break;
            }
    ...
    void loadShroomieButton(float difficulty)
    {
        ...
        {
            OnShroomieUpdateCost?.Invoke(Math.Max(ShroomieBaseCost,
                (AccumulatedShroomies * 100) + (AccumulatedShroomies + 1) * 50));
        }
    }
    ...
    public void OnEnemyKill(int amount)
    {
        ...
        OnUpdateMulch?.Invoke(AccumulatedMulch);
    }
    ...
    public void onPlayerDeath()
    {
        ...
        _gameOverEffect.transform.Find("PulseEffect").position = new Vector3(pulseEffect.position.x,
            GameObject.FindWithTag("Player").transform.position.y, pulseEffect.position.z);
        setPlayerControls(false);
    ...
    {
        OnInvokeGameOver?.Invoke(false); // Hides game over text if game over
        ...
        Transform timeElapsedText = _resultsScreen.transform.Find("TimeElapsed").transform.Find("Number"),
            ...
            titleText = _resultsScreen.transform.Find("Title");
        timeHeader.GetComponent<TextMeshProUGUI>().text = won ? "CLEAR TIME" : "TIME ELAPSED";
        titleText.GetComponent<TextMeshProUGUI>().text = won ? "VICTORY" : "MUSHED";
        ...
        IEnumerator StatsDisplayCoroutine()
        {
            ...
            for (int i = 0;
                 i < _clearTime;
                 i += (i + (_clearTime / 64) > _clearTime ? 1 : Mathf.Clamp(i / 64, 1, (i / 64) + 1)))
            {
            ...
            if (_enemiesKilled > 0)
            {
                for (int i = 0;
                     i <= _enemiesKilled;
                     i += (i + (_enemiesKilled / 64) > _enemiesKilled ? 1 : Mathf.Clamp(i / 64, 1, (i / 64) + 1)))
                {
            ...
            if (_earnedMulch > 0)
            {
                for (int i = 0;
                     i <= _earnedMulch;
                     i += (i + (_earnedMulch / 64) > _earnedMulch ? 1 : Mathf.Clamp(i / 64, 1, (i / 64) + 1)))
                {
                    mulchEarnedText.GetComponent<TextMeshProUGUI>().text = i.ToString();
                    float adjustedVolume = Mathf.Clamp(.01f / (_earnedMulch * .01f), 0.06f, .25f);
                    AudioManager.Instance.PlaySFX("Tick", adjustedVolume);
    ...
    IEnumerator ThankYouScreen()
    {
        ...
        yield return new WaitUntil(() =>
            InputManager.InputActions.UI.Enter.WasPressedThisFrame() || Input.touchCount > 0);
    ...
    string getTime(int seconds)
    {
        ...
        int m = seconds / 60;
        s = seconds % 60;
        return normalizeToTwoDigits(h.ToString()) + ":" + normalizeToTwoDigits(m.ToString()) + ":" +
               normalizeToTwoDigits(s.ToString());
    }
    ...
    bool userClickedOrTappedScreen()
    {
        return (Input.touchCount > 0 && Input.GetTouch(0).phase == TouchPhase.Began) ||
               InputManager.InputActions.UI.Enter.WasPressedThisFrame();
    }
}